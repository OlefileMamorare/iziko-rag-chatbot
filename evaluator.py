...
def get_color(value: float, metric_type: str) -> str:
    ...
    if metric_type == "coverage":
        return (
            "green" if value >= COVERAGE_GREEN else "orange" if value >= COVERAGE_AMBER else "red"
        )
...
def main():
    ...
    with gr.Blocks(title="Iziko RAG Evaluation Dashboard", theme=theme) as app:
        gr.Markdown("# Iziko RAG Evaluation Dashboard")
        gr.Markdown("Evaluate retrieval and answer quality for the Iziko Cloud Solutions RAG system.")
